...
### 6. Merkle-tree anti-entropy — what actually goes into a leaf, where the tree lives, and how often it's rebuilt

**A leaf hash has to include the value (or its vector clock), not just the key — otherwise the tree is blind to a modified existing key.** Hash just the key (or a sorted key list) and the tree can only tell you "this key exists in one replica and not the other" — additions and deletions, nothing else. The correct construction is `leaf_hash = hash(sorted (key, value, vector_clock) tuples in this bucket)`. Any modification to an existing key changes its vector clock, which changes the leaf hash, which changes every ancestor hash up to the root — a modified key is indistinguishable, at comparison time, from a brand-new one. The tree doesn't need to know which case it is; it only needs to localize *where* the two replicas disagree. A follow-up step — exchanging the actual `(key, value, vector_clock)` data for whichever leaf bucket disagreed — is what determines addition, deletion, or update, and reconciles it via the same vector-clock dominance rule used everywhere else in this design (Hard Problem 3).

**Internal-node hashing:** `hash(left_child_hash || right_child_hash)`, fixed left-right order (never sort children before combining, or the mapping back to which half of the key range diverged is lost). Same hash function as the leaves, just different input — no need for two algorithms. One good-practice refinement worth naming: prepend a 1-byte domain-separation tag (`0x00` for leaves, `0x01` for internal nodes) before hashing, so a leaf's raw input can never be crafted to collide with a combined-hash input. Not strictly required here (both sides of an anti-entropy comparison are trusted nodes in the same cluster, not mutually distrusting parties, unlike Certificate Transparency or a blockchain), but free to add.

**Which hash function:** SHA-1 (Cassandra's original, real-world choice — 160-bit, negligible collision probability) or, for lower CPU cost at scale, a fast non-cryptographic 128-bit hash like xxHash128 or MurmurHash3-128. This isn't an adversarial context, so cryptographic strength isn't the requirement — what matters is a large enough output space and good distribution that a collision (silently hiding a real divergence, a correctness bug) is vanishingly unlikely across a dataset that can run into billions of keys cluster-wide.

**Storage: not a separate database — a derived, recomputable index built from the same SSTables the node already has.** Adding a persisted, independently-maintained tree store would be new distributed state to keep durable and consistent, which this design avoids everywhere else (no central Rule DB, no central Policy DB, Part 4). Scope is one tree per key-range (vnode) per replica pair, not one tree for a node's whole dataset and not one tree per individual key. If persisted at all, it's a local on-disk cache (flat array / binary-heap layout, since a tree over a fixed range has a fixed shape) purely to avoid recomputation between rounds — always safe to discard and rebuild from the SSTables, which is what makes it a cache and not a second source of truth.

**Rebuild cadence: piggybacked on compaction, not a full rebuild on every anti-entropy round, and not per-write incremental maintenance either.** Compaction already does a full linear pass over every key in a range to merge SSTables — recomputing leaf hashes during that same pass reuses work already happening rather than adding a second scan. Between compactions, the cached tree is reused across however many comparisons occur. Compaction is also the point where a key's current state finally settles into one canonical on-disk representation (Part 3's write/read path: before compaction, a key's true value may be scattered across the memtable and several SSTables) — the cheapest, least ambiguous moment to hash from. True per-write incremental maintenance (update one leaf + its O(log N) ancestor path on every write) is a real alternative, but it taxes the write path with extra work on every write, fighting the entire reason an LSM engine was chosen (Part 3: a cheap, append-only write path, expensive work deferred to the background). One addition beyond the ambient compaction cadence: when gossip announces a node rejoining after being marked down, that should trigger an immediate rebuild (forcing a memtable flush first, so the freshly built tree reflects everything durable) rather than waiting for the next scheduled compaction, which could be hours away.

**Complexity, for N leaf buckets (depth O(log N)):** building the tree from scratch is O(N) — total nodes ≈ 2N−1, each hashed once, the geometric sum collapsing to linear, not O(N log N). Space is O(N) fixed-size hashes. Comparing two already-built trees: O(1) if fully in sync (root comparison ends it); O(log N) for exactly one divergent leaf (a single root-to-leaf path); O(D log N) as a simple upper bound for D divergent leaves, though shared ancestors reduce the real cost below that; the hard ceiling regardless of D is O(N) — even total divergence never costs worse than touching every node's hash once, each comparison being a cheap fixed-size hash check, not a full data comparison.

**Worked example — 12 leaf buckets, bucket size 3 (36 keys covered).** Bucket B*i* holds keys `3i-2, 3i-1, 3i`. Building bottom-up with 12 leaves (not a power of 2 — the one odd level pairs its lone node with itself to stay strictly binary):

```
Level 0 (leaves):  L1  L2  L3  L4  L5  L6  L7  L8  L9  L10 L11 L12
Level 1:           N1=h(L1,L2)   N2=h(L3,L4)   N3=h(L5,L6)   N4=h(L7,L8)   N5=h(L9,L10)  N6=h(L11,L12)
Level 2:           M1=h(N1,N2)          M2=h(N3,N4)          M3=h(N5,N6)
Level 3:           P1=h(M1,M2)                         P2=h(M3,M3)   <- odd one out, paired with itself
Level 4 (root):                    Root=h(P1,P2)
```

Say `K14` (existing key, in B5) gets a value update, and `K26` (brand-new key, in B9) gets created, while node D is down. Comparing D against a healthy replica E: `Root` differs → `P1` and `P2` **both** differ (the two changes are in different branches — this is the case where divergence is spread out, not localized, and both subtrees need visiting). Under `P1`: `M1` matches (B1-B4 untouched, skip that whole subtree), `M2` differs → descend → `N3` differs, `N4` matches (skip) → descend → `L5` differs (found — B5), `L6` matches (skip). Under `P2` (`h(M3,M3)`, so a mismatch here directly implicates `M3`, no redundant compare needed): `M3` differs → `N5` differs, `N6` matches (skip) → `L9` differs (found — B9), `L10` matches (skip). Result: exactly B5 and B9 identified in ~10 small hash comparisons, without ever touching B1-B4, B6-B8, or B10-B12. D and E then exchange only those two buckets' actual data (6 of 36 keys, 17%) — `K14` reconciled by vector-clock dominance (a genuine update), `K26` simply copied over (doesn't exist on D at all, nothing to reconcile).

**Why this mechanism exists here at all, and wouldn't in a different architecture:** Merkle-tree comparison is the answer to a specific problem — reconciling two replicas that evolved independently, with no single trusted counter either side can check against. That's a direct consequence of this design's AP, leaderless choice (Part 1): any node can coordinate a write, conflicts are tracked with vector clocks, and there is no consensus-backed global ordering. A simple `version > checkpoint` delta query would be far cheaper if it were viable — but it requires exactly the thing this design deliberately doesn't have: one source of truth handing out a single, globally-ordered counter both sides can trust. Content-hash comparison is what works without that.
...
9. Explains what actually goes into a Merkle leaf hash (value/vector-clock, not just the key) and why that's what makes it catch modified keys, not only new ones — rather than treating "Merkle tree" as a black box that "syncs stuff."
